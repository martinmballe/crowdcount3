...
def main(args):
    ...
    img_list = sorted(glob.glob(os.path.join(data_dir, mode, 'images', '*.jpg')))
    print(f"Looking for images in: {os.path.join(data_dir, mode, 'images', '*.jpg')}")
    ...
    for device, img_list in enumerate(sub_list):
        for file in img_list:
            ...
            # Debug: Check the ground-truth path construction
            gt_file = file.replace('images', 'ground-truth').replace('jpg', 'mat')
            print(f"Looking for ground-truth in: {gt_file}")
            locations = loadmat(gt_file)['annPoints']
